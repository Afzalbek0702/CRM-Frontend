...
// 			<div className="absolute bottom-0 left-0 h-0.5 w-0 bg-linear-to-r from-primary/50 to-transparent transition-all duration-200 group-hover:w-full"></div>
// 		</div>
// 	);
// }
...
export default function StatsCards({
	...
	const getIcon = () => {
		...
		if (type.includes("Qarzdorlar") || type.includes("kelmaganlar"))
			return <AlertCircle size={iconSize} />;
		if (type.includes("O'quvchilar") || type.includes("Lidlar"))
	...
	return (
		<div
			className={`relative group p-6 rounded-2xl border ${colorMap[color]} transition-all duration-200 hover:scale-[1.02] hover:shadow-lg`}
		>
			<div className="flex justify-between items-start">
				...
				{percentage !== undefined && (
					<span
						className={`text-xs font-bold px-2 py-1 rounded-full ${trend === "up" ? "bg-green-500/20 text-green-500" : "bg-red-500/20 text-red-500"}`}
					>
			...
			<div className="mt-4">
				<h4 className="text-xs font-semibold opacity-60 uppercase tracking-widest">
					{type}
				</h4>
				<div className="flex items-baseline gap-2 mt-1">
					...
					{type.includes("Tushum") && (
						<span className="text-sm opacity-50">UZS</span>
					)}
				</div>
				{description && (
					<p className="text-[10px] mt-2 opacity-40 italic">{description}</p>
				)}
			</div>
